...
// export default function Passage({ passage, input, started, finished, keyErrors, heatmapKey, category, startTest }) {
...
//                     {/* Text content */}
//                     <div className={`transition-all ${!started ? "blur-[1px]" : ""} ${isCode ? "font-mono text-green-400 bg-gray-900 p-4 rounded-lg whitespace-pre-wrap overflow-x-auto" : "font-sans text-2xl leading-[1.8] tracking-wide text-gray-800 dark:text-gray-200"}`}>
//                         {!isCode ? (
                ...
                {started && (
                    <div className="mt-6 flex items-center justify-center space-x-4">
                        <div className="flex items-center gap-2">
                            <TbProgress className="w-4 h-4 text-blue-400" />
                            <span className="text-sm font-medium text-gray-400">Progress</span>
                        </div>
                        <div className="flex-1 max-w-md h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                            <div
                                className="h-full bg-linear-to-r from-blue-500 to-emerald-500 rounded-full transition-all duration-300 relative"
                                style={{ width: `${(input.length / passage.length) * 100}%` }}
                            >
                                ...
                                return (
                                    <span key={i} className={bgClasses}>
                                        {isCaret && (
                                            <span className={`absolute inset-0 -mx-px -my-0.5 rounded pointer-events-none animate-pulse ${isError ? "bg-rose-400/30" : "bg-blue-400/30"}`}>
                                                <span className={`absolute left-0 top-0 w-0.5 h-full ${isError ? "bg-rose-500" : "bg-blue-500"}`} />
                                                <HiOutlineSparkles className="absolute -top-2 -right-2 w-3 h-3 text-blue-400" />
                                            </span>
                                        )}
                                        <span className={charClasses}>{char === ' ' ? '\u00A0' : char}</span>
                                    </span>
